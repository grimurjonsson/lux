...
mod tests {
    ...
    #[test]
    fn test_no_args() {
        let cli = Cli::try_parse_from(["ctail"]).unwrap();
    ...
    #[test]
    fn test_single_rule() {
        let cli = Cli::try_parse_from(["ctail", "-r", "ERROR:red"]).unwrap();
    ...
    fn test_multiple_rules() {
        let cli = Cli::try_parse_from(["ctail", "-r", "ERROR:red", "-r", "WARN:yellow"]).unwrap();
        assert_eq!(cli.rules.len(), 2);
    }

    #[test]
    fn test_color_always() {
        let cli = Cli::try_parse_from(["ctail", "--color", "always"]).unwrap();
        assert!(matches!(cli.color, ColorChoice::Always));
    }

    #[test]
    fn test_color_never() {
        let cli = Cli::try_parse_from(["ctail", "--color", "never"]).unwrap();
    ...
    fn test_profile_flag() {
        let cli = Cli::try_parse_from(["ctail", "--profile", "django"]).unwrap();
        assert_eq!(cli.profile.as_deref(), Some("django"));
    }

    #[test]
    fn test_config_flag() {
        let cli = Cli::try_parse_from(["ctail", "--config", "/tmp/my.toml"]).unwrap();
        assert_eq!(cli.config.as_deref(), Some("/tmp/my.toml"));
    }

    #[test]
    fn test_list_profiles_flag() {
        let cli = Cli::try_parse_from(["ctail", "--list-profiles"]).unwrap();
        assert!(cli.list_profiles);
    }

    #[test]
    fn test_list_colors_flag() {
        let cli = Cli::try_parse_from(["ctail", "--list-colors"]).unwrap();
    ...
    fn test_follow_descriptor() {
        let cli = Cli::try_parse_from(["ctail", "-f", "app.log"]).unwrap();
    ...
    #[test]
    fn test_bare_file() {
        let cli = Cli::try_parse_from(["ctail", "app.log"]).unwrap();
        assert_eq!(cli.file.as_deref(), Some("app.log"));
        assert!(!cli.follow_descriptor);
        assert!(!cli.follow_name);
        assert!(cli.lines.is_none());
    ...
    #[test]
    fn test_n_with_file() {
        let cli = Cli::try_parse_from(["ctail", "-n", "20", "app.log"]).unwrap();
    ...
    fn test_n_plus_syntax() {
        let cli = Cli::try_parse_from(["ctail", "-n", "+5", "app.log"]).unwrap();
        assert_eq!(cli.lines.as_deref(), Some("+5"));
    }

    #[test]
    fn test_n_with_follow() {
        let cli = Cli::try_parse_from(["ctail", "-n", "5", "-f", "app.log"]).unwrap();
        assert_eq!(cli.lines.as_deref(), Some("5"));
        assert!(cli.follow_descriptor);
    }
    ...
    fn test_f_and_f_upper_conflict() {
        let result = Cli::try_parse_from(["ctail", "-f", "-F", "app.log"]);
    ...
    fn test_f_without_file_allowed() {
        ...
        let cli = Cli::try_parse_from(["ctail", "-f"]).unwrap();
        assert!(cli.follow_descriptor);
    ...
    fn test_trigger_single() {
        let cli = Cli::try_parse_from(["ctail", "--trigger", "ERROR"]).unwrap();
    ...
    fn test_trigger_multiple() {
        let cli =
            Cli::try_parse_from(["ctail", "--trigger", "ERROR", "--trigger", "WARN"]).unwrap();
    ...
    fn test_trigger_with_before_after_count() {
        let cli = Cli::try_parse_from([
            "ctail", "--trigger", "ERROR", "--trigger", "WARN", "--before", "5", "--after", "10",
        ])
        .unwrap();
        assert_eq!(cli.trigger, vec!["ERROR", "WARN"]);
    ...
    fn test_trigger_with_before_after_pattern() {
        let cli = Cli::try_parse_from([
            "ctail", "--trigger", "ERROR", "--before", "^===", "--after", "^---",
        ])
        .unwrap();
    ...
    fn test_profile_new_subcommand() {
        let cli = Cli::try_parse_from(["ctail", "profile", "new"]).unwrap();
        assert!(matches!(
            cli.command,
    ...
    fn test_profile_new_with_config() {
        let cli = Cli::try_parse_from(["ctail", "profile", "new", "--config", "/tmp/my.toml"]).unwrap();
        match cli.command {
    ...
    fn test_profile_edit_subcommand() {
        let cli = Cli::try_parse_from(["ctail", "profile", "edit", "django"]).unwrap();
        match cli.command {
    ...
    fn test_profile_delete_subcommand() {
        let cli = Cli::try_parse_from(["ctail", "profile", "delete", "django"]).unwrap();
        match cli.command {
    ...
    fn test_profile_list_subcommand() {
        let cli = Cli::try_parse_from(["ctail", "profile", "list"]).unwrap();
        assert!(matches!(
            cli.command,
